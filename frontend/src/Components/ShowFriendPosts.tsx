...
import { Trash2, Delete, Bookmark, Share2, SquarePen, SendHorizontal, ThumbsUp, MessageCircleMore, ClockIcon } from 'lucide-react';
...
export default function ShowFriendPosts() {
    const { authUser } = useAuthStore();
    const navigate = useNavigate();
    const [isLike, setIsLike] = useState<boolean>(false);
    const [showComment, setShowComment] = useState<boolean>(false);
    const {
        friendPosts, deletePost, isDeletingPost, setIsUpdatingPost, isDeletingComment, deleteComment, isBookMarke,
    ...
    return (
        <div className="mt-14 pb-4">
            <h1 className="text-[30px] sm:text-[40px] font-bold mb-2 text-primary">All Posts</h1>
            <div className="flex flex-col gap-8">
                {friendPosts && friendPosts.length > 0 && friendPosts.map((post) => (
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <Link onClick={(e) => {
                                    ...
                                    e.stopPropagation()
                                }} to={authUser?._id == post.userId._id ? '/profile' : `/user/${post.userId._id}`} className="size-10 self-start">
                                    <img
                                ...
                                <div
                                    ...
                                    <div className="flex items-center gap-4">
                                        <Link onClick={(e) => {
                                            ...
                                            e.stopPropagation()
                                        }} to={authUser?._id === comment.user._id ? '/profile' : `/user/${comment.user._id}`} className="size-10 self-start">
