...
export default function ShowMyPosts() {
    const { authUser } = useAuthStore();
    const [isLike, setIsLike] = useState<boolean>(false);
    const [showComment, setShowComment] = useState<boolean>(false);
    const {
        myPosts, deletePost, isDeletingPost, setIsUpdatingPost, isDeletingComment, deleteComment, isBookMarke,
    ...
    return (
        <div className="mt-14 pb-4">
            <h1 className="text-[40px] font-bold mb-2 text-primary">My Posts</h1>
            <div className="flex flex-col gap-8">
                {myPosts && myPosts.length > 0 && myPosts.map((post) => (
                    <div onMouseEnter={() => {
                        ...
                        key={post._id} className="bg-secondary p-4 rounded-md shadow-md">
                        {/* User Info */}
                            ...
                            <div className="flex items-center gap-4">
                                <div className="size-10 self-start">
                                    <img
                                        ...
                                        src={authUser?.profilePic || defaultImage} alt="User Image" />
                                </div>
                        ...
                        <div className="flex items-center justify-between mt-4">
                            <div className="flex items-center gap-4">
                                <div onClick={() => handleLike(post._id)} className={`flex gap-1 items-center cursor-pointer ${authUser && post.likes?.includes(authUser._id) ? 'text-blue-500' : 'text-gray-500'} `}>
                                    <ThumbsUp
